...
export const updateActualState = async (req, res) => {   
    ...
    try {
        await ActualStateModel.update(req.body, { 
            where: {id: actualStateId}
        });
        
        const actualStateUpdated = await ActualStateModel.findOne({
            where: {id: actualStateId},
            raw: true
        });
        
        res.status(200).json(actualStateUpdated);
    } catch(error) {
...
export const deleteActualState = async (req, res) => {
    const actualStateId = req.params.id;
    try {
        const actualState = await ActualStateModel.findOne({ 
            where: { id: actualStateId },
            raw: true 
        });
        
        if (!actualState) {
            return res.status(404).json({ message: 'Estado actual no encontrado' });
        }
