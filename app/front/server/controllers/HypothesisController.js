...
export const updateHypothesis = async (req, res) => {   
    ...
    try {
        await HypothesisModel.findOne({where: {id: hypothesisId}});
        if (!hypothesisId) {
            return res.status(404).json({ message: 'Hipótesis no encontrada' });
        }
        await HypothesisModel.update(req.body,{  where: {id: hypothesisId}});
        const updatedHypothesis = await HypothesisModel.findOne({ where: { id: hypothesisId } });
        res.status(200).json(updatedHypothesis);
    } catch(error) {
        res.status(500).json({message: error.message});
    }   
}
