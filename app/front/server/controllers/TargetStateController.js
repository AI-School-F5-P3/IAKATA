...
export const deleteTargetState = async (req, res) => {
    ...
    try {
        ...
        if (!targetState) {
            return res.status(404).json({ message: 'Estado objetivo no encontrado' });
        }

        await TargetStateModel.destroy({ where: { id: targetStateId } });
        res.status(200).json({ id: targetStateId, deleted: true });
    } catch (error) {
...
export const updateTargetState = async (req, res) => {
    ...
    try {
        ...
        const updatedTargetState = await TargetStateModel.findOne({ where: { id: targetStateId } });
        res.status(200).json(updatedTargetState);
    } catch (error) {
